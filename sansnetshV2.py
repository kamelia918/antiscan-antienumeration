            ...
            if packet.is_inbound:
                ...
                if src_ip in suspect_ips and time.time() < suspect_ips[src_ip]:
                    ...
                    if packet.icmp:
                        ...
                        print(f"Paquet bloqué de {src_ip} vers le port {dst_port}")
                ...
                # Bloquer immédiatement si un scan est détecté
                if len(ports_scanned[src_ip]) > 0 and suspect_ips.get(src_ip, 0) >= 1:
                    print(f"Scan détecté de {src_ip} sur les ports: {ports_scanned[src_ip]}")
                    tarpit(src_ip)
                    block_ip(src_ip)
